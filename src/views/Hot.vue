			...
			<div v-for="(content, index) in tabContent" v-show="cur == index" :key="index">
				<div class="card border-bottom fill-white" v-for="(item, index1) in content" :key="index1">
					<div v-if="!item.children[0].thumbnail" class="d-flex p-3">
						...
						<div class="ml-3">
							<a :href="'https://www.zhihu.com/question/' + item.target.id" target="_blank" class="bl-sub-title ellipsis-1">{{ item.target.title }}</a>
							<p class="mt-2 ellipsis-4 half-more-height mr-3">{{ item.target.excerpt }}</p>
