		...
		<div class="cards">
			<div class="card-roundtable" v-for="(item,index) in roundtable" :key="index">
				<div class="talk" :style="{'background-image':'url('+item.banner+')'}">
					<p class="bl-meta-000000">{{ item.name}}</p>
					<p class="bl-meta-000000min">{{ item.visitsCount }}次浏览</p>
				</div>
			</div>
		</div>
	</div>
...
<template>
	<div class="container fill-white border tiny-round">
		...
		<div class="d-flex flex-wrap pl-5 pt-3">
			<div class="col-3 mb-3" v-for="(item, index) in roundtable" :key="index">
				<div class="mask"></div>
				<img :src="item.tinyBanner" />
