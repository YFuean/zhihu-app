<template>
	<div>
		<div v-for="(item, index) in followList" :key="index" class="card p-3 fill-white tiny-round  border-bottom">
			<div class="row ml-2">
				<img :src="item.target.author.avatar_url" alt="">
				<h5 class="ml-3 mt-2">{{item.target.author.name}}</h5>
			</div>
			<h3 v-if="item.target.question == null" class="mb-2 mt-2 ml-2">{{ item.target.title }}</h3>
			<h3 v-else class="mt-2 mb-2 ml-2">{{ item.target.question.title }}</h3>
			<div class="p-2" v-if="item.target.thumbnail">
				<div class="fruid-col-3 mr-3"><img :src="item.target.thumbnail" class="fill tiny-round" /></div>
				<p class="fruid-col-9 ellipsis-4">{{ item.target.excerpt }}</p>
				<span class="deep-blue link">阅读全文</span>
				<svg class="deep-blue-icon link" viewBox="0 0 24 24" width="24" height="24">
			...
			<div class="p-2 mb-4" v-else>
				<p class="fruid-col-12 ellipsis-2">{{ item.target.excerpt }}</p>
				<span class="deep-blue link">阅读全文</span>
				<svg class="deep-blue-icon link" viewBox="0 0 24 24" width="24" height="24">
					<path d="M12 13L8.285 9.218a.758.758 0 0 0-1.064 0 .738.738 0 0 0 0 1.052l4.249 4.512a.758.758 0 0 0 1.064 0l4.246-4.512a.738.738 0 0 0 0-1.052.757.757 0 0 0-1.063 0L12.002 13z"
					 fill-rule="evenodd"></path>
